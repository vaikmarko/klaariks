...
| State | Description |
|-------|-------------|
| **All good** | All documents processed, no low-confidence items, no missing transactions |
| **Needs attention** | Missing document, low-confidence extraction, unmatched payment, inconsistent data |
| **Blocked / unclear** | Critical uncertainty (e.g. VAT treatment unclear), system requires owner input before proceeding |
...
Company: id, registry_code, name, vat_registered, created_at
Membership: id, company_id, user_id, role (owner, accountant_readonly, viewer), created_at
```
...
AuditEvent:
  ...
  event_type (upload, parse, suggestion_created, suggestion_accepted, 
...
### Access & users
- Accountant access is invite-only by email
- Accountant access in early versions is information handoff only (exports), not in-app editing
- Multi-user and multi-company accounts are future v1+, but data model must support them

### Company onboarding
- Company creation should use Estonian business registry lookup (required for real customers)
- Fiscal year customization is out of scope for v0

### Accounting logic
- Source of truth when invoice and bank data disagree is open (to be defined with accountant)
- Chart of accounts strategy is open and must be defined with accounting partner
- Rules for editing parsed invoice fields by owners are open

### Accountant handoff
v0 handoff format: ZIP archive containing:
- Original documents
- Ledger export
- Journal export
- VAT summary preview

Incremental / diff-based exports are open.

### Integrations & automation
- v0 supports manual imports only (banks, invoices)
- Bank integrations are v1
- Unique inbound invoice email per company is v1
- Estonian e-invoice (e-arve) support is future, not v0

### OCR & AI
- OCR latency of minutes is acceptable if accuracy is high
- Balance between OCR + AI extraction vs pure AI is open (hallucination risk acknowledged)
- Confidence should be communicated via labels (High / Medium / Low) initially
- If system confidence is low, the system must always ask the owner before proceeding

### Commercial & support
- Billing and pricing enforcement are out of scope for v0
- Nano-tier users will receive human support initially, growth is expected to be gradual
